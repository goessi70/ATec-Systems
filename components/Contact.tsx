...
const ContactContent: React.FC<{ isDark: boolean; lang: Language }> = ({ isDark, lang }) => {
  ...
  const contactInfo = [
    ...
    { icon: 'fa-envelope', label: t.labels.email, val: 'info@atec-systems.ch', href: 'mailto:info@atec-systems.ch' },
    { icon: 'fa-location-dot', label: t.labels.headquarters, val: 'Allrütiweg 9, CH-6343 Rotkreuz', href: null }
  ];
        ...
        <div className="flex flex-col lg:flex-row gap-20">
          <div className="lg:w-1/3 space-y-12">
            <div>
              ...
              <h2 className={`text-xl sm:text-2xl md:text-4xl font-black mb-8 tracking-tight sm:tracking-normal whitespace-nowrap ${isDark ? 'text-white' : 'text-slate-900'}`}>{t.title}</h2>
            ...
            <div className="space-y-6">
              {contactInfo.map(item => (
                <div key={item.label} className={`flex items-start gap-6 p-6 rounded-2xl border ${
                  ...
                  <div className="overflow-hidden">
                    <div className="text-[9px] uppercase font-bold text-slate-500 tracking-widest mb-1">{item.label}</div>
                    {item.href ? (
                      <a href={item.href} className={`font-bold block transition-colors hover:text-blue-500 whitespace-nowrap overflow-hidden text-ellipsis text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>
                        {item.val}
                      </a>
                    ) : (
          ...
          <div className={`lg:w-2/3 p-12 rounded-[3rem] border transition-all ${
            isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-2xl shadow-blue-500/5'
          }`}>
            {submitted ? (
              <div className="text-center py-20 animate-fadeIn">
                ...
                <h3 className={`text-2xl font-black mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t.successTitle}</h3>
                <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t.successDesc}</p>
